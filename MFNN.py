...
from ml import naive_bayes, xgboost, rdforest
...
class MFNN():
    ...
    def build(self):
        ...
        for hidden_layer in hiddens:
            model.add(Dense(
                hidden_layer["units"], activation=hidden_layer["activation"], name=hidden_layer["name"]))
        # Output Layer
        ...
        # Compile the model
        model.compile(optimizer=optimizers,  # self.hyperparams["optimizer"],
    ...
    def train(self):
        ...
        auc_maxes = {"Neural Network": [None, None, 0],
    ...
    def plot_auc(self, auc_maxes, file):
        ...
        for k, v in auc_maxes.items():
            plt.plot(v[0], v[1],
                     label='ROC curve {} (area = {:.2f})'.format(k, v[2]))
        plt.plot([0, 1], [0, 1], 'k--')
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
